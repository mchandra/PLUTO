 ...
 *********************************************************************** */
{
  ...
  double maxrate;
  double dqheat, *dx1, *dx2, *dx3, dvol, vol;
  double sendarray[2], recvarray[2];
  intList var_list;
  
  dqheat = 0.0; vol = 0.0;
  dx1 = GXYZ[IDIR].dx; dx2 = GXYZ[JDIR].dx;
...
/*  SolveODE_ROS34 (v0, k1, v1, dtsub, min_tol);  */
  ...
  /* -- pressure must be positive -- */
    ...
    if (prs < g_smallPressure) prs = g_smallPressure;
  ...
  /* ---- Update solution array ---- */
    ...
    dvol = dx1[i]*dx2[j];
    dqheat += (d->Vc[PRS][k][j][i] - prs)*dvol; 
    vol += dvol;
  ...
  #ifdef PARALLEL
   sendarray[0]=dqheat; sendarray[1]=vol; 
   MPI_Allreduce (sendarray, recvarray, 2, MPI_DOUBLE, MPI_SUM,MPI_COMM_WORLD);
   dqheat=recvarray[0]; vol=recvarray[1];
  #endif

  dqheat /= vol;
  DOM_LOOP(k,j,i){
    d->Vc[PRS][k][j][i] += dqheat;
  }
}
