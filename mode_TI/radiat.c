#include "pluto.h"
#define frac_Z   3.e-5   /* = N(Z) / N(H), fractional number density of 
                              metals (Z) with respect to hydrogen (H) */ 
#define frac_He  0.085   /* = N(Z) / N(H), fractional number density of 
...
//    QUIT_PLUTO(1);
    if (T < T_tab[klo]) khi = klo+1;
    if (T > T_tab[khi]) klo = khi-1;
  }
  ...
  rhs[RHOE] *= E_cost*UNIT_DENSITY*UNIT_DENSITY/(1.17*1.304*CONST_mp*CONST_mp);
}
#undef T_MIN
/* ******************************************************************* */
